...
const LoginScreen = () => {
    ...
    return (
        <div className="min-h-dvh w-full flex items-center justify-center flex-col gap-4" style={{
            ...
            <h1 className="text-4xl text-white text-center"> Hey User! welcome back </h1>
            <div className="min-h-[60vh] min-w-[280px] bg-white w-1/3 rounded-md shadow p-3 flex flex-col items-center justify-center gap-3">
                ...
                <div className=''>
                    <input type="checkbox" value={String(isConfirmedTerms)} onChange={(e) => setIsConfirmedTerms(Boolean(e.target.value))} className='mx-2' /><span>I agree for your terms and conditions.</span>
                </div>
                <button onClick={handleSubmitForm} disabled={isConfirmedTerms == false} className="bg-sky-600 p-2 px-5 text-white rounded-md shadow-sm cursor-pointer disabled:opacity-70">Connect Now</button>
                <p>Not you have an account ? <Link className="text-sky-600 font-bold" to={"/signup"}>Sign up</Link></p>
            </div>
            {
